...
import { NewsItemDetail, NewsDefaultDetail } from '@/components/news/NewsItemDetail';

export default function NewsDetailClient({news, otherNews}) {
  ...
  // Type filter state
  const allTypes = Array.from(new Set(otherNews.map(e => e.type).filter(Boolean)));
  ...
  const now = Date.now();
  let filteredNews = otherNews;
  if (dateFilter !== 'all') {
    filteredNews = otherNews.filter(e => {
      const newsTime = parseDate(e.time || e.date);
      if (dateFilter === 'past_month') {
        return newsTime > now - 30 * 24 * 60 * 60 * 1000;
      } else if (dateFilter === 'past_2_months') {
        return newsTime > now - 2 * 30 * 24 * 60 * 60 * 1000;
      } else if (dateFilter === 'past_5_months') {
        return newsTime > now - 5 * 30 * 24 * 60 * 60 * 1000;
      }
      return true;
    });
  }
  // Filter by type
  if (typeFilter !== 'all') {
    filteredNews = filteredNews.filter(e => e.type === typeFilter);
  }
  ...
  return (
    <main className="max-w-5xl mx-auto p-8">
      ...
      { currentPath === '/news' ? (
        <NewsDefaultDetail news={news} />
      ) : (
        <NewsItemDetail news={news} />
      )}
        ...
        <div className="mt-10">
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {sortedNews.map(e => {
              ...
              return (
                <Link key={e.id} href={`/news/${e.id}`} className="block bg-blue-50 rounded-lg p-4 shadow hover:bg-blue-100 transition relative">
                  {/* Type tag */}
                  {e.type && (
                    <span
                      className={
